...
7. In the terminal in the project directory, run `docker volume create shapeshifters-data`. This creates the database for the web application.
8. Run `docker compose -f docker-compose-dev.yaml up --build`.
9. Run `docker exec -it shapeshifters-fastapi-1 bash` to open a shell in the FastAPI container.
10. Once inside the shell, run `python db_script.py` to populate the events database with sample events and sample users.
10. Type `exit` to close the shell.
11. To close the web application, run `docker compose -f docker-compose.dev.yaml down`

If you want to remove all named volumes declared in the compose file along with anonymous volumes attached to containers, run `docker volume rm shapeshifters-data`. You will have to rerun the database creation step the next time you wish to use this application locally.
